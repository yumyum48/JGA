...
void MonsterMove() {
	...
	if (g_enemybeat < ENEMY_BEAT_MAX[g_select_Stage]) {
		EnemyMove();								// 弱い敵の動き
		PlaySoundMem(g_sound[BGM_BOSS], DX_PLAYTYPE_LOOP, FALSE);		// 雑魚戦のＢＧＭ再生
	}
	else {
		StopSoundMem(g_sound[BGM_BOSS]);								// 雑魚戦のＢＧＭを停止
		SetCurrentPositionSoundMem(0, g_sound[BGM_BOSS]);				// 雑魚戦のＢＧＭの再生開始位置を最初に戻す
		PlaySoundMem(g_sound[BGM_BATTLE], DX_PLAYTYPE_LOOP, FALSE);		// ボス戦のＢＧＭの再生
