...
void Boss_Knock_Down() {
	...
	if (g_boss[g_select_Stage].hp <= 0) {
		...
		StopSoundMem(g_sound[BGM_BATTLE]);						// ボス戦のＢＧＭを停止
		SetCurrentPositionSoundMem(0, g_sound[BGM_BATTLE]);		// ボス戦のＢＧＭの生成開始位置を最初に戻す
...
void BossEnemyDropDisp() {
	...
	PlaySoundMem(g_sound[BOSS3_ATTACK2], DX_PLAYTYPE_BACK, TRUE);			//雑魚敵の召喚のSE
}
// 弱い敵を出す(動き(当たり判定など))
