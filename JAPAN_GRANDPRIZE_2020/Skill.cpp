...
void SkillDisp_1(int aniMAX, int aniMIN) {		//間合い伸びるやつ（player.cppのPlayerIntervalに書いてある）
	...
	if (g_skillAniFlg[0] == TRUE) {
		...
		if (++g_skillAnime[0] >= 50) g_skillAniFlg[0] = FALSE, g_skillAnime[0] = 0;
	}
}
...
void SkillDisp_2(int aniMAX, int aniMIN) { //飛ばすやつ
	...
	if (g_skillAniFlg[1] == TRUE) {
		...
		else g_skillAniFlg[1] = FALSE, g_skillAnime[1] = 0, g_skill_Y = 0;
	}
...
void SkillDisp_7(int aniMAX, int aniMIN) { //分身
	...
	if (g_player.attackFlg == TRUE) {
		g_skillAniFlg[6] = TRUE;
	}

	if (g_skillAniFlg[6] == TRUE) {
		...
		if (++g_skillAnime[6] >= 50) g_skillAniFlg[6] = FALSE, g_skillAnime[6] = 0;
	}
	// スキル中.
	//EnemyLockOn();
